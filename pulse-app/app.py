...
def start_audit_run(domain,pages,scope):
    job=enqueue_job(RESEARCH_DB,"geo_aeo_audit",domain=domain,payload={"domain":domain,"scope":scope,"page_ids":[int(p["id"]) for p in pages if p["id"] is not None]},max_attempts=3)
    return job["id"]
...
def sync_dashboard_domains_from_opengsc():
    with research_db() as suppression_con:
        suppressed_domains = {
            row["domain"].lower()
            for row in suppression_con.execute(
    ...
    with research_db() as con:
        for gsc in gsc_sites:
            ...
            if domain.lower() in suppressed_domains:
                continue
            con.execute(
                """INSERT INTO dashboard_domain(
                       domain,base_url,gsc_site_id,created_at,updated_at
                   ) VALUES (?,?,?,?,?)
...
def manual_ai_source_analyze(domain):
    ...
    def f(name,default,lo,hi):
        try: v=float(request.form.get(name,str(default)))
        except Exception: v=default
        return max(lo,min(hi,v))
    try: max_tokens=int(request.form.get("analysis_max_output_tokens","8000"))
    except Exception: max_tokens=8000
    try: prompt_version=int(request.form.get("analysis_system_prompt_version","3"))
    except Exception: prompt_version=3
    prm={"provider":provider,"model":request.form.get("analysis_model","").strip(),"reasoning_effort":effort,
         "temperature":f("analysis_temperature",.1,0,2),"top_p":f("analysis_top_p",.9,0,1),
         "max_output_tokens":max(256,min(128000,max_tokens)),"system_prompt_version":max(1,prompt_version),
         "question_set_version":int(source.get("question_set_version") or 1),
         "system_prompt":request.form.get("analysis_system_prompt","").strip()}
    job=enqueue_job(RESEARCH_DB,"manual_ai_analysis",domain=domain,payload={"domain":domain,"source_snapshot":source,"parameters":prm},max_attempts=3)
    return redirect(url_for("manual_ai_source",domain=domain,message=f"AI analysis queued · job {job['id']}"))
...
@app.get("/api/jobs")
def durable_jobs_index():
    domain = request.args.get("domain")
    status = request.args.get("status")
    try:
        limit = int(request.args.get("limit", "50"))
        jobs = list_jobs(RESEARCH_DB, domain=domain, status=status, limit=limit)
    except ValueError as exc:
